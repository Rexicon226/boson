//! Utilities for abstracting finite fields.
const std = @import("std");
const Flatcode = @import("Flatcode.zig");
const curves = @import("curves.zig");

const F641 = curves.F641;
...
pub fn Qap(Field: type) type {
    return struct {
        ...
        pub fn fromFlat(flat: Flatcode, allocator: std.mem.Allocator) !Q {
            ...
            for (flat.instructions, 0..) |inst, i| {
                ...
                switch (inst.op) {
                    ...
                    .add => {
                        ...
                        Flatcode.setVar(a, inst.lhs, variables);
                        Flatcode.setVar(a, inst.rhs, variables);
                        b[0] = 1;
                    },
                    .mul => {
                        c[variables.getIndex(inst.dest).?] = 1;
                        Flatcode.setVar(a, inst.lhs, variables);
                        Flatcode.setVar(b, inst.rhs, variables);
                    },
                }
            }
        ...
        pub fn solutionPolynomials(
            ...
            var S = try M.initCoerce(allocator, r, r.len);
...
test "basic qap" {
    ...
    const qap = try Qap(curves.F641).fromFlat(flat, allocator);
    defer qap.deinit(allocator);

    try expectEqualFe(F641, &.{
        636, 116, 636, 535,
        8,   416, 5,   213,
        0,   0,   0,   0,
        635, 330, 637, 321,
...
test "polynomial add" {
...

    {
        ...
        try expectEqualFe(F641, &.{ 3, 5, 4 }, x.coeffs.items);
    }
...
test "polynomial mul" {
...

    {
        ...
        try expectEqualFe(F641, &.{ 2, 7, 10, 8 }, x.coeffs.items);
    }

    {
        ...
        try expectEqualFe(F641, &.{ 2, 7, 14, 12 }, x.coeffs.items);
    }
}
